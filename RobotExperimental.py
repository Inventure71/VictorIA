...
class RobotHandling:
	...
	self.distance_between_cells = 0.031
        ...
        self.self.adjustment_multiplier = 0.0025 

        robot_startup()
        
    def move_to_safe_location(self):
        self.bot.arm.set_ee_pose_components(self.safe_location[0], self.safe_location[1], self.safe_location[2], pitch=self.vertical_pitch))
	...
	self.move_to_safe_location()
    	...
    	self.bot.arm.set_ee_pose_components(self.tower_coords[0], self.tower_coords[1], self.tower_coords[2]+0.1, pitch=self.vertical_pitch)
    	time.sleep(0.1)
    	self.bot.arm.set_ee_pose_components(self.tower_coords[0], self.tower_coords[1], self.tower_coords[2], pitch=self.vertical_pitch)
